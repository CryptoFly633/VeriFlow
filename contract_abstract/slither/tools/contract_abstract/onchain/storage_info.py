...
class StorageInfo:
    ...
    def create_init_tables(self):
        ...
        simple_attributes.append(("block_number", "BIGINT")) # 用于记录storage状态是那个block的
        simple_attributes.append(("block_hash", "VARCHAR(66)")) # 用于记录storage状态是那个block的hash
        simple_attributes.append(("current_block_number", "BIGINT")) # 用于记录当前storage状态的最新block的number
        simple_attributes.append(("current_block_hash", "VARCHAR(66)")) # 用于记录当前storage状态最新block的hash
        self.create_table(self.simple_table_name, simple_attributes) # 使用统一的id BIGSERIAL PRIMARY KEY
    ...
    def create_table_for_mapping(self, table_name, entity):
        ...
        # 使用统一的id BIGSERIAL PRIMARY KEY，key1作为UNIQUE约束
        attributes.append(("block_number", "BIGINT"))
        attributes.append(("block_hash", "VARCHAR(66)"))
    ...
    def get_current_block_number(self):
        """
        从数据库中获取当前区块号，从simple_table_name表的最新记录中获取current_block_number
        
        Returns:
            int: 当前区块号，如果没有记录则返回0
        """
        ...
        try:
            # 查询simple_table_name表中最新的current_block_number
            query = f"""
            SELECT current_block_number 
            FROM {self.simple_table_name} 
            WHERE current_block_number IS NOT NULL 
            ORDER BY __id DESC 
            LIMIT 1
            """
            ...
            if result and result[0] is not None:
                ...
                logger.info(f"从数据库获取当前区块号: {current_block}")
                return current_block
            else:
                logger.info("数据库中未找到current_block_number记录，返回0")
                return 0
    ...
    def sync_storage_to_block(self, block_number): # 一个交易一个交易的同步storage
        ...
        current_block = block
        transactions_gen = self.transaction_info.get_transactions_paginated(block, block_number, page_size=1000)    
        for transactions in transactions_gen:
            for tx in transactions:
                if tx["block_number"] > current_block:
                    tx_index = 0
                elif tx["block_number"] == current_block:
                    tx_index += 1
                else:
                    raise Exception(f"交易区块号小于当前区块号: {tx['block_number']} < {current_block}")
                current_block = tx["block_number"]
                logger.info(f"同步区块: {current_block} 的第{tx_index}个交易")
    ...
    def create_table(self, table_name, attributes, primary_keys=None):
        if not self.db_connection:
            self.connect_db()
        ...
        # 添加统一的id字段作为主键
        attributes_str = ["__id BIGSERIAL PRIMARY KEY"]
        ...
        # 如果有额外的唯一约束，添加UNIQUE约束
        unique_constraints = []
        if primary_keys and len(primary_keys) > 0:
            # 为指定的primary_keys添加UNIQUE约束
            unique_constraints.append(f"UNIQUE ({', '.join(primary_keys)})")
